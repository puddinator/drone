...
#include <EEPROM.h> // memory whose values are kept when the board is turned off (like a tiny hard drive). This library enables you to read and write those bytes.

#include <vector> // Vectors are same as dynamic arrays with the ability to resize itself automatically when an element is inserted or deleted, with their storage being handled automatically by the container.
#include <algorithm> // The algorithms library defines functions for a variety of purposes (e.g. searching, sorting, counting, manipulating) that operate on ranges of elements.
 ...
 * 
 * The 0x part tells the compiler that what follows is a number in hexadecimal.
 * So the address of 0x68 is the bit pattern 0110 1000.
*/
...
// to modify the way in which the compiler and subsequent program treats the variable.
...
// storing the centre position of the sticks
...
// uint16_t is unsigned 16-bit integer. unsigned short int is unsigned short integer, 
// but the size is implementation dependent. The standard only says it's at least 16-bit.
...
// typedef is a keyword in C and C++ which lets you create custom data types, or more accurately: create an alias name for another data type. 
// Keep in mind that it does not create a new type, but instead adds a new name for some existing type.
 ...
 * millis()  counts the number of milliseconds the sketch has been running.
 * |
 */
  ...
  while (timer > millis() && zero < 15)  {
    ...
    // if reciever sends any value between 900 and 2100 (stick is somewhat centred), then zero =! 0, which would mean that the reciever is recieving
    delay(500);
    Serial.print(F("."));
...
void register_min_max() {
  ...
  //records down any (both high and low) values from reciever

  while (receiver_input[1] < center_channel_1 + 20 && receiver_input[1] > center_channel_1 - 20) {
    //Serial.println(receiver_input[1]);
    delay(250); 
    //check if channel 1 centered, i.e. check if stick is within ± 20 of the previously saved value.
    //proceed to next loop if sticks are centered
  }
  ...
  while (zero < 15) {
    ...
    // setting the low_channel as the minimum value
    ...
    // setting the high_channel as the maximum value
  ...
  Serial.println(F(""));
  Serial.println(F(""));
  Serial.println(F("LOW - CENTRE - HIGH values found during setup"));
...
void calibrate_accel() {
  ...
  int32_t axis[6] = {3, 2, 2, 1, 1, 3}; //(original)
  
  //changed:
  //int32_t axis[6] = {3, 1, 1, 2, 2, 3};

  // original:   int32_t axis[6] = {3, 2, 2, 1, 1, 3};
  ...
  for (int i = 0; i < 6; i++) {
    ...
    //display message for "Z axis LOW"
    for (int j = 5; j > 0; j--) {
      Serial.print(j);
      Serial.print(" ");
      //countdown from 5
    ...
    for (int k = 0; k < 2000; k++) {
      ...
      //every 200 milliseconds? (basically 2000/200 times)
      ...
      acc_x_sum += acc_x; // adding all the acceleartion up
      acc_y_sum += acc_y; // summation
      acc_z_sum += acc_z;
    }
    Serial.println();
    if (axis[i] == 1) {
      acc_x_sum /= 2000;
      //divide total sum of acceleration by 2000 to get average acceleration
      Serial.println(acc_x_sum);
      axis[i] = acc_x_sum;
      // save new averate acceleartion as the axis[0] first
    } else if (axis[i] == 2) {
      acc_y_sum /= 2000;
 ...
 * 
 * i=0: Z axis LOW , saves the z value
 * i=1: X axis LOW , saves the x value
 * i=2: X axis HIGH , saves the x value
 * i=3: Y axis LOW , saves the y value
 * i=4: Y axis HIGH , saves the y value
 * i=5: Z axis HIGH, saves the z value
 */
